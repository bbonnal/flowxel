...
public class GraphExecutor<TNode> where TNode : class, IExecutableNode
{
    ...
    /// <summary>
    /// Executes nodes and collects results.
    /// </summary>
    /// <typeparam name="TResult">The type of result returned by each node.</typeparam>
    /// <param name="executeFunc">A function that executes a node and returns a result.</param>
    /// <param name="cancellationToken">A token to cancel the execution.</param>
    /// <returns>A dictionary mapping node IDs to their execution results.</returns>
    public async Task<IReadOnlyDictionary<Guid, TResult>> ExecuteWithResultsAsync<TResult>(
        Func<TNode, CancellationToken, Task<TResult>> executeFunc,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(executeFunc);

        var results = new Dictionary<Guid, TResult>();
        var inDegree = _graph.Nodes.ToDictionary(n => n.Id, n => _graph.GetInDegree(n.Id));
        var ready = new Queue<TNode>(_graph.Nodes.Where(n => inDegree[n.Id] == 0));
        ...
        while (ready.Count > 0)
        {
            ...
            var tasks = currentBatch.Select(async node => 
            {
                var result = await executeFunc(node, cancellationToken);

...
public abstract class ExecutionNode<TIn, TOut> : IExecutableNode where TOut : notnull
{
    private readonly ResourcePool _pool;
    private readonly Graph<IExecutableNode> _graph;

    protected ExecutionNode(ResourcePool pool, Graph<IExecutableNode> graph)
    {
        _pool = pool;
        _graph = graph;
    }
    ...
    private IReadOnlyList<TIn> ResolveInputs()
    {
        ...
        foreach (var inputPort in inputPorts)
        {
            ...
            if (edges.Length != 1)
                throw new InvalidOperationException($"Input port '{inputPort}' on node {Id} expects exactly one source but received {edges.Length}.");
